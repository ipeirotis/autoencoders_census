...
### ~~3.2 Support custom model configs for uploaded data~~ DONE
Added a model preset system that gives upload-pipeline users both a manual choice and an auto-select fallback. Both modes ship together: the dropdown defaults to **Auto** (the recommended option), and users can explicitly pick **Small / Medium / Large** when they want more direct control.

- **`model/presets.py`** — new pure-Python module that owns the preset catalog plus the public API: `PRESET_INFO` (dropdown metadata), `PRESET_DEFINITIONS` (concrete hyperparameter dicts for small/medium/large), `normalize_preset_name()` (liberal coercion of arbitrary input to a valid id), `auto_select_preset(input_dim, n_rows)` (heuristic that picks small/medium/large from the cleaned dataset shape), `get_preset_config()`, and `build_model_config()` (the high-level entry point used by the worker / Vertex container). The auto-select breakpoints are exposed as module constants (`AUTO_SELECT_MIN_INPUT_DIM_FOR_MEDIUM=30`, `AUTO_SELECT_MIN_ROWS_FOR_MEDIUM=200`, `AUTO_SELECT_MIN_INPUT_DIM_FOR_LARGE=150`, `AUTO_SELECT_MIN_ROWS_FOR_LARGE=1000`) so tests can reference them without duplicating magic numbers.

- **Preset shapes** — small is a 1-layer encoder (units=32, latent_dim=4, epochs=10) for tiny CSVs; medium is a 2-layer encoder (units=128/64, latent_dim=8, epochs=15) — the typical-case default; large is a 3-layer encoder with L2 / batch-norm regularization (units=256/128/64, latent_dim=16, epochs=25) for wide datasets with enough rows to fit the extra capacity. Each preset bakes in `epochs` and `batch_size` so the worker no longer needs hardcoded training-loop knobs.

- **Worker integration (`worker.py`)** — `PubSubMessage` gained an optional `modelPreset` field (Pydantic v2 ignores extras by default, so old workers still parse new messages). `process_upload_local` and `process_upload_vertex` both accept a `model_preset` kwarg, log requested-vs-resolved values, and persist `modelPreset` + `modelPresetRequested` on the Firestore job doc so the frontend can show "model: medium (auto)" on the results page. The hardcoded `model_config = {...}` block was replaced by `build_model_config(model_preset, input_dim, n_rows)` in both code paths. `keras_model.fit(...)` now reads `epochs` / `batch_size` from the resolved preset instead of the old `epochs=15, batch_size=32` literals. `_build_vertex_training_args()` gained an optional `model_preset` parameter that emits `--model-preset=<name>` only when supplied (preserving the rolling-deploy contract that an older `train/task.py` parses the arg list cleanly).

- **Vertex container (`train/task.py`)** — accepts the new `--model-preset` CLI flag, threads `model_preset` through `train_and_predict`, calls the same `build_model_config` helper for parity with the local worker, and writes `modelPreset` / `modelPresetRequested` alongside the final result document.

- **Express API (`frontend/server`)** — `validateStartJob` (`middleware/validation.ts`) now allowlists `modelPreset` against `VALID_MODEL_PRESETS = ['auto', 'small', 'medium', 'large']`. `routes/jobs.ts` extracts the preset from the request body, defaults missing values to `'auto'`, persists `modelPresetRequested` on the Firestore doc at job-creation time, and forwards the field through the Pub/Sub message. Added `GET /api/jobs/presets` (public, no auth — static catalog) that returns the preset metadata for the frontend dropdown; the route is registered before any `/:id` patterns so a future `GET /:id` route cannot accidentally swallow it. The TypeScript preset list lives in `frontend/server/utils/modelPresets.ts` and mirrors `PRESET_INFO`.

- **Frontend UI (`frontend/client`)** — `Index.tsx` fetches `/api/jobs/presets` on mount (with a `FALLBACK_PRESETS` constant for offline dev), renders a labeled `<select>` dropdown next to the upload button, defaults to `auto`, and surfaces the selected preset's description as helper text. The selected preset id is forwarded to `uploadCsv(file, modelPreset)` → `startJob(jobId, gcsFileName, modelPreset)` → the new `modelPreset` body field. `JobProgress.tsx` shows "Model preset: medium (auto)" on the progress page. `JobStatus` and the polling hook expose `modelPreset` / `modelPresetRequested` so other UI surfaces can read them.

- **Tests**:
  - `tests/test_model_presets.py` (70 tests, no TF dependency) — covers the static catalog (preset_info ids match valid_presets, definitions have required keys, widths increase with size), `normalize_preset_name` (canonical / case-normalized / whitespace / unknown / non-string inputs), `auto_select_preset` (small / medium / large breakpoints, zero-row / zero-input edge cases), `get_preset_config` (fresh-copy semantics, sentinel rejection, unknown-name rejection), `build_model_config` (named pass-through, all three auto-resolution buckets, None / unknown fallback, copy semantics, resolved-name is always concrete), `worker.PubSubMessage` (optional field, presence / absence / unknown values), `worker._build_vertex_training_args` (preset flag omitted when None, included when supplied, coexists with `--max-unique-values`), and a drift guard that fails if the TypeScript preset list or the Express validation allowlist drift from the Python `PRESET_INFO` / `VALID_PRESETS` constants.
